...
router.post("/upload-chunk/:id",checkUploadAuth, upload.single("chunk"), async (req, res) => {
    ...
    if (status === 'start') { // first chunk of the file
        try {
            ...
            consoleLogOut(applicationName,req_sessionId,`Cleared old chunks for ${originalName} in ${sessionUploadDir}`);
        } catch (err) {
            consoleErrorOut(applicationName,req_sessionId,`Error clearing old chunks: ${err.message}`);
    ...
    if (parseInt(checksum) !== computedChecksum) {
        consoleErrorOut(applicationName,req_sessionId,`Checksum mismatch for chunk ${chunkIndex} of ${originalName}`);
    ...
    consoleLogOut(applicationName,req_sessionId,`Received chunk ${parseInt(chunkIndex)+1}/${totalChunks} for ${originalName}.Checksum:${computedChecksum}`);
    ...
    if (status == 'end' || status == 'single') { // proceed to merge
        ...
        consoleLogOut(applicationName,req_sessionId,`"${status}" flag received,Total:${parseInt(totalChunks)}, Received: ${receivedChunks}, Filename: ${originalName}`);
        if (receivedChunks !== parseInt(totalChunks)) {
            consoleErrorOut(applicationName,req_sessionId, `Mismatch: Expected ${totalChunks} chunks, but received ${receivedChunks}`);
            return res.status(400).json({ error: "Incomplete upload, please retry" });
        }

        try{
            consoleLogOut(applicationName,req_sessionId,`Start merging ${originalName}.`);
            await mergeChunks(originalName, totalChunks,sessionUploadDir,nas_mount_location,req_sessionId); // proceed to merge
        }catch (error){
            consoleErrorOut(applicationName,req_sessionId,`Sending back 400, uploaded failed, SMB issue`)
...
async function mergeChunks(originalName, totalChunks, sessionUploadDir,nas_mount_location, req_sessionId) {
    ...
    for (let i = 0; i < totalChunks; i++) {
        ...
        if (!fs.existsSync(chunkPath)) {
            consoleErrorOut(applicationName,req_sessionId, `Chunk ${i} for ${originalName} missing, aborting merge.`);
    ...
    writeStream.end();
    consoleLogOut(applicationName,req_sessionId, `File ${originalName} successfully assembled.`);
    ...
    try {
        ...
        consoleLogOut(applicationName,req_sessionId, `${originalName} moved to destination at ${destinationPath}`);
    } catch (error) {
        consoleErrorOut(applicationName,req_sessionId, `${originalName} move to destination failed:`, error);
        throw new Error(`${originalName} move to destination failed`);
    }
}
...
async function getUniqueFilename(originalFilename, nas_mount_location, req_sessionId) {
    ...
    try {
        ...
        while (fs.existsSync(path.join(nas_mount_location, uniqueFilename))) {
            ...
            consoleLogOut(applicationName,req_sessionId, `Name conflict, trying name: ${uniqueFilename}`);
            counter++;
        }
    } catch (error) {
        consoleErrorOut(applicationName,req_sessionId, `Failed to access destination directory:`, error);
        throw new Error(`Failed to access destination directory to get unique filename.`);
    }
...
router.use((err, req, res, next) => {
    ...
    //   console.log("Error middleware triggered. Params:", req.body.req_sessionId, "Body:", req.body);
      consoleErrorOut(applicationName,req.body.req_sessionId, `Chunk size exceeds ${chunkSize}MB limit: ${err.message}`);
      return res.status(400).json({ error: `Chunk too large. Max allowed size is ${chunkSize}MB.` });
    }
    next(err);
  });
